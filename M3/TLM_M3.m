...
InputParasL.wg= 5e-13;  % Gaussian width 
InputParasL.phi= 0; % phase 
InputParasR= struct(); % No input is implemented for the right source 

InputParasR.E0= 0;  % Amplitude
InputParasR.we= 0;    % Angular Frequency
InputParasR.t0= 0;  % Time offset 
InputParasR.wg= 1;  % Gaussian width (Avoiding zero-division)
InputParasR.phi= 0; % phase 


% The source function with modulation
function E = SourceFct_M3(t, InputParas)
...
%d Define grating kappa 
kappa_0 = 100;
kappa = kappa_0 * ones(size(z));
kappa(z < 0.4*L) = 0; 
kappa(z < 0.6*L) = 0;
...
xlabel('z (cm)');
ylabel('kappa');
title('The plotting of kappa as a function of z');
...
for i = 2:Nt
    ...
    InputL(i) = SourceFct_M3(t, InputParasL);
    InputR(i) = SourceFct_M3(t, InputParasR);
...
%----The codes that have been removed to focus on working on miletsone 3----
